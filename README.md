...
- 🕸️ 知识图谱（概念 → 笔记可视化）
- 🧭 **概念管理**：知识图谱 key 的本地增删改（新增/编辑/删除 + 删除引用保护 + 导出 `concepts.yaml` 回电脑端 `graph build`）——校验与 CLI 一致，改动存本机 IndexedDB
- 🏷️ 自由标签 + 概念标签（知识图谱概念），批量统计/重命名/删除
- 📷 拍照/选图插入（自动压缩 ≤900px，存入本地）
- 🔄 导入 / 导出 JSON（多终端数据搬运），导入支持**差异对比与合并策略**（智能合并/仅新增/完整替换），导入前自动备份
