...
- 🔠 **阅读字号**：详情页题头「Aa」四档（小/标准/大/特大，存本机；详情/图谱面板/预览同源）＋ 流式字号随屏幕自适应；桌面宽屏自动放宽版面（详情正文栏保持舒适行长）
- 📖 **书页模式**：「◫ 视图」切换 连续滚动 / 双页并排翻页（宽屏双列、窄屏单页，键盘 ←/→ 翻页）
- 🗑 概念管理支持**删除域**（级联删概念+清除引用，删除前自动备份）；图谱图例自动隐藏空域；页脚显示缓存版本号（更新是否生效一眼可见）
